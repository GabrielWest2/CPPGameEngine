...
namespace objl
{
	...
	struct Mesh
	{
		...
		{
			Vertices = _Vertices;//  - Y increase down
			Indices = _Indices;
			/*
			//Flip y direction when loading   - Y increase up
			Vertices = std::vector<Vertex>();
			for (int i = 0; i < _Vertices.size(); i++) {
				Vertex vCopy = _Vertices[i];
				vCopy.position.y *= -1;
				Vertices.push_back(vCopy);
			}
	...
	private:
			...
			std::string icurline)
		{
			...
			for (int i = 0; i < int(sface.size()); i++)
			{
				...
				{
					vVert.position = algorithm::getElement(iPositions, svert[0]);
					vVert.texUV = algorithm::getElement(iTCoords, svert[1]);
					noNormal = true;
					oVerts.push_back(vVert);
					break;
				}
		...
		void VertexTriangluation(std::vector<GLuint>& oIndices,
